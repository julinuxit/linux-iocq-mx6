...
static void ov5647_standby(s32 enable)
{
	...
	printk(KERN_ERR"ov5647_mipi_camera_powerdown: powerdown=%x, power_gp=0x%x\n", enable, pwn_gpio);
	msleep(2);
}
...
static s32 update_device_addr(struct sensor_data *sensor)
{
	...
	if (ret < 0)
		pr_err("%s: ov5647_mipi ret=%d\n", __func__, ret);
	else printk(KERN_ERR"[%s:%s:%d] Update OK\n",__FILE__,__FUNCTION__,__LINE__);
	return ret;
}
...
static void ov5647_reset(void)
{
	...
	update_device_addr(&ov5647_data);
...
static s32 ov5647_write_reg(u16 reg, u8 val)
{
	...
	if ((reg == 0x3008) && (val & 0x80)) {
		...
		ret = i2c_master_send(ov5647_data.i2c_client, au8Buf, 3);
				...
				enum ov5647_mode mode)
{
	...
	if ((binning_on()) && (mode != ov5647_mode_720P_1280_720)
			&& (mode != ov5647_mode_1080P_1920_1080))
		prev_shutter *= 2;
	...
	/* get average */
	ov5647_read_reg(0x56a1, &average);
...
#if 0
static int ov5647_download_autofocus(void)
{
	u8  r;
	int sval = ov5647_read_reg(0x3000, &r);
	if (0 > sval) {
	...
	r |= 0x20;
	ov5647_write_reg(0x3000,r);
	sval = ov5647_download_firmware(
		ov5647_af_firmware,
		ARRAY_SIZE(ov5647_af_firmware));
	if (0 > sval) {
	...
	/* VCM control4 */
	ov5647_write_reg(0x3606,0x07);
	return sval;
}

static int trigger_auto_focus(void){
        ov5647_write_reg(CMD_ACK, 0x01);
	ov5647_write_reg(CMD_MAIN, 0x03);
	return 0;
}
#endif
	...
	switch (vc->id) {
		case 105: //step to specified position
			...
			if(vc->value0 < 0 || vc->value0 > 255)
				return ret;
			loca_val = vc->value0;
			retval1 = ov5647_read_reg(0x3602,&regval);
			if (0 > retval1) {
				pr_err("ov5647_read_reg(3602): %d\n", retval1);
				return retval1;
			}
			regval &= 0x0f;
			regval |= (loca_val&7) << 5; 	/* low 3 bits */
			ov5647_write_reg(0x3602, regval);
			ov5647_write_reg(0x3603, loca_val >> 3);
			ret = 0;
			break;
		default:
			    ...
			    enum ov5647_mode mode, enum ov5647_mode orig_mode)
{
	...
	if (ov5647_data.pix.pixelformat == V4L2_PIX_FMT_SBGGR8){
		...
		printk(KERN_ERR"Current Sensor Format Code: 0x%X\n", ov5647_data.pix.pixelformat);
	...
	if (mode == ov5647_mode_INIT) {
		//pModeSetting = ov5647_init_setting_30fps_VGA;
		//ArySize = ARRAY_SIZE(ov5647_init_setting_30fps_VGA);
        ...
        /*        retval = ov5647_download_autofocus();
		if (retval < 0) {
		...
		*/
		...
		//pModeSetting = ov5647_setting_30fps_720P_1280_720;
		//ArySize = ARRAY_SIZE(ov5647_setting_30fps_720P_1280_720);
		retval = ov5647_download_firmware(pModeSetting, ArySize);
...
printk(KERN_ERR"[%s:%s:%d] --------------------------------------\n", __FILE__, __FUNCTION__, __LINE__);
	...
	ov5647_data.pix.pixelformat = V4L2_PIX_FMT_SBGGR10;
	ov5647_data.pix.width = 640;
	ov5647_data.pix.height = 480;
	//ov5647_data.pix.width = 1280;
	//ov5647_data.pix.height = 720;
